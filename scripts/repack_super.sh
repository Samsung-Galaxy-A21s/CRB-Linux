...
sudo umount Projects/$PROJECT/Build/system
sudo umount Projects/$PROJECT/Build/product
sudo umount Projects/$PROJECT/Build/vendor
sudo umount Projects/$PROJECT/Build/odm
...
./tools/super_to_raw/lpmake --metadata-size 65536 \
    ...
    --super-name super \
    --metadata-slot 1 \
    --device super:5557452800 \
    --group main:$MAIN_SIZE \
    --partition system:readonly:$CALCULATED_SYSTEM_SIZE:main \
    --image system=$_OUTPUT/system.img \
    --partition product:readonly:$CALCULATED_PRODUCT_SIZE:main \
    --image product=$_OUTPUT/product.img \
    --partition vendor:readonly:$CALCULATED_VENDOR_SIZE:main \
    --image vendor=$_OUTPUT/vendor.img \
    --partition odm:readonly:$CALCULATED_ODM_SIZE:main \
