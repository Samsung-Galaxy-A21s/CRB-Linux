...
void newProject() {
    ...
    snprintf(command0, sizeof(command0), "[ -d \"./Projects/%s/\" ] && echo \"[Error 1] Project already exists\" && exit", name);
    system(command0);
...
void unpackSuper() {
    ...
    snprintf(command4, sizeof(command4), "[ ! -d \"Projects/%s/\" ] && echo \"[Error 1] This Projects does NOT exist!\" && exit", project);
    system(command4);

	char superPath[1000];
	printf("\nSpecify the FULL path to the folder where the super.img is located!\n");
	printf("PATH: ");
	scanf("%s", &superPath);

	char command5[10000]; // Adjust the size as needed
    snprintf(command5, sizeof(command5), "[ ! -f \"%ssuper.img\" ] && echo \"[Error 1] Super.img not found!\" && exit", superPath);
    system(command5);
